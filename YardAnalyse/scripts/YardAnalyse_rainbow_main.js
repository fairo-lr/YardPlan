...
function GetAreaTable1(area) {
    jQuery.ajax({
        type: "POST",
        datatype: "JSON",
        url: "YardAnalyse_ashx/Handler.ashx",
        data: { "area": area, "bay": null },
        success: function (output) {
            if (output != "") {
                SetAreaTableHTML1(output);
            }
        }
    });
}

function SetAreaTableHTML1(table) {
    //设置箱区HTML代码 
